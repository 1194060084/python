...
def model_svr(train_x, train_y, test_x, test_y):
    print('运行SVR模型')
    svr = SVR()
...
def model_lasso(train_x, train_y, test_x, test_y):
    print('运行LASSO模型')
...
def model_krr(train_x, train_y, test_x, test_y):
    print('运行KRR模型')
    krr = KernelRidge()
    krr.fit(train_x, y=train_y)
    pre_y = krr.predict(test_x)
    # print('pre_y = ', pre_y)
    map_plot(test_x,test_y,pre_y)
    guji(pre_y,test_y)
    print('************************************')
...
if __name__ == '__main__':
    ...
    model_svr(train_x, train_y, test_x, test_y)
    model_lasso(train_x, train_y, test_x, test_y)
    model_rfr(train_x, train_y, test_x, test_y)
    model_krr(train_x, train_y, test_x, test_y)
